...
Tine.Billing.BatchJobMonitionItemEditDialog = Ext.extend(Tine.widgets.dialog.EditDialog, {
	
	/**
	 * @private
	 */
	windowNamePrefix: 'BatchJobMonitionItemEditWindow_',
	appName: 'Billing',
	recordClass: Tine.Billing.Model.BatchJobMonitionItem,
	recordProxy: Tine.Billing.batchJobMonitionItemBackend,
	loadRecord: false,
	evalGrants: false,
	batchJobMonitionRecord: null,
	initComponent: function(){
		this.on('load',this.onLoadBatchJobMonitionItem, this);
		Tine.Billing.BatchJobMonitionItemEditDialog.superclass.initComponent.call(this);
	},
	onLoadBatchJobMonitionItem: function(){
		if(!this.rendered){
			this.onLoadBatchJobMonitionItem.defer(250,this);
		}
		if(this.batchJobMonitionRecord){
			Ext.getCmp('workflow_sub_elem_workflow_elem_id').setValue(this.batchJobMonitionRecord.get('id'));
			this.record.set('workflow_elem_id', this.batchJobMonitionRecord.get('id'));
		}
	},
	/**
	 * returns dialog
	             ...
	             [
					{
						...
						fieldLabel: 'BatchJobMonitionItement-Nr',
 ...
 * Billing Edit Popup
 */
Tine.Billing.BatchJobMonitionItemEditDialog.openWindow = function (config) {
    var id = (config.record && config.record.id) ? config.record.id : 0;
    var window = Tine.WindowFactory.getWindow({
        width: 600,
        height: 560,
        name: Tine.Billing.BatchJobMonitionItemEditDialog.prototype.windowNamePrefix + id,
        contentPanelConstructor: 'Tine.Billing.BatchJobMonitionItemEditDialog',

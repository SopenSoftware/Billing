<?php
class Billing_Backend_BatchJobDta extends Tinebase_Backend_Sql_Abstract
{
    /**
     ...
     * @var string
     */
    protected $_tableName = 'bill_batch_dta';
     ...
     * @var string
     */
    protected $_modelName = 'Billing_Model_BatchJobDta';
    ...
    {     
        ...
        $select->joinLeft(array('bact' => $this->_tablePrefix . 'bill_bank_account'),
			$this->_db->quoteIdentifier($this->_tableName.'.bank_account_id') . ' = ' . $this->_db->quoteIdentifier('bact.id'),
			array()
		);
		...
		$select->joinLeft(array('sepa' => $this->_tablePrefix . 'bill_sepa_mandate'),
			$this->_db->quoteIdentifier($this->_tableName.'.sepa_mandate_id') . ' = ' . $this->_db->quoteIdentifier('sepa.id'),
			array()
		);
